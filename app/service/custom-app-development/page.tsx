...
export default async function CustomAppDevPage() {
  const data = await sanityClient.fetch(serviceBySlugQuery, { slug: "custom-app-development" });

  return (
    <main className="mx-auto max-w-4xl px-6 py-16 space-y-6">
      <h1 className="text-3xl md:text-4xl font-bold">{data?.title ?? "Custom App Development"}</h1>
      {data?.image && (
