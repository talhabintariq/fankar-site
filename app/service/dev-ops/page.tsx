...
export default async function DevOpsPage() {
  // Make sure your Sanity doc slug is exactly "dev-ops"
  const data = await sanityClient.fetch(serviceBySlugQuery, { slug: "dev-ops" });

  if (!data) {
    return (
      <main className="mx-auto max-w-4xl px-6 py-16 space-y-6">
        <h1 className="text-3xl md:text-4xl font-bold">Dev Ops</h1>
        <p className="text-muted-foreground">
          Content coming soon. Please create/publish the “Dev Ops” service in Sanity with slug <code>dev-ops</code>.
        </p>
      </main>
    );
  }
